...
class PhotoBoothApp:
	def __init__(self):
		...
		self.frame_settings = tki.Frame(self.canvas, height=680, width=2500)
		self.frame_settings.grid(column=0)
        
		self.canvas.config(width=1100, height=630)
		...
		lbl8 = tki.Label(self.frame_settings, text="Номер изображения")
		...
		btn3.grid(row=12, column=0, sticky='N', padx = 25, pady = 8)
		...
		self.pic.grid(row = 1, column = 2, sticky='N', padx=10, pady=2)

		self.img_results = [tki.Label(self.frame_settings), tki.Label(self.frame_settings), tki.Label(self.frame_settings), tki.Label(self.frame_settings), tki.Label(self.frame_settings)]
		for i in range(len(self.img_results)):
			self.img_results[i].grid(row = 2, column = i+2, sticky='N', padx=10, pady=2)
